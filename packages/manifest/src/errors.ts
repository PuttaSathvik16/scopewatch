 ...
 */
function getRecoverySuggestion(error: ValidationError): string | null {
  ...
  if (path === 'schemaVersion' && code === 'invalid_literal') {
    return (
      `  Fix: This tool only supports schemaVersion: 1.\n` +
  ...
  if (path === 'version' && code === 'invalid_string') {
    return (
      `  Fix: version must be semantic versioning (e.g., "1.2.0")\n` +
  ...
  if (code === 'invalid_union' || error.message.includes('Required')) {
    ...
    if (field === 'tool_id') {
      return (
        `  Fix: Every capability must declare which tool provides it.\n` +
    ...
    if (field === 'verb') {
      return (
        `  Fix: Every capability must declare a verb: one of\n` +
  ...
  if (code === 'invalid_string' && error.message.includes('URL')) {
    return (
      `  Fix: homepage must be a valid URL (e.g., "https://example.com")\n` +
 ...
 */
export function renderValidationError(errors: ValidationError[]): string {
  if (errors.length === 0) {
    return '✓ Manifest is valid.';
  }
