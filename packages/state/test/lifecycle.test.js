...
function seedManifest(db, server_id, version) {
...
test('CHECK constraints reject invalid enum values', () => {
    ...
    throws(() => {
        db.prepare(`INSERT INTO lockfile_entries (server_id, client_id, manifest_id, state, updated_at)
       VALUES (?, ?, ?, ?, ?)`).run('test-server', 'claude-code', manifestId, 'bogus-state', 0);
...
test('Server can be pushed through every lifecycle state and back', () => {
    ...
    for (const [from, to] of path) {
        const intentId = engine.startTransition(server_id, client_id, from, to, null);
    ...
    strictEqual(intents.length, 8, 'eight transitions attempted');
    strictEqual(confirmed.length, 8, 'eight transitions confirmed');
    db.close();
});
// ---------------------------------------------------------------------------
...
test('startTransition rejects a second transition while one is unresolved', () => {
    ...
    throws(() => engine.startTransition('test-server', 'claude-code', 'discovered', 'reviewed', null), /Transition already in progress/, 'second transition should be rejected while first is unresolved');
...
test('Recovery runs automatically before the first transition on a fresh engine instance', () => {
    ...
    db1
        .prepare(`INSERT INTO lifecycle_events (server_id, client_id, from_state, to_state, event_type, created_at)
...
test('rollback restores the last checkpoint and records a rollback event', () => {
    ...
    for (const [from, to] of [
        ...
        const id = engine.startTransition(server_id, client_id, from, to, null);
...
test('rollback refuses when a transition is unresolved (invariant check)', () => {
    ...
    throws(() => engine.rollback(server_id, client_id), /transition is still unresolved/, 'rollback should refuse while a transition is unresolved');
    db.close();
});
test('rollback fails clearly when no checkpoint exists', () => {
    ...
    throws(() => engine.rollback('test-server', 'claude-code'), /No checkpoint available/, 'rollback should fail with an actionable message, not a raw exception');
    db.close();
});
// ---------------------------------------------------------------------------
...
test('Crash point 2: kill after intent+checkpoint commit, before confirmation (active -> update)', () => {
    ...
    for (const [from, to] of [
        ...
        const id = engine1.startTransition(server_id, client_id, from, to, null);
    ...
    const intentRow = db1.prepare('SELECT checkpoint_id FROM lifecycle_events WHERE id = ?').get(intentId);
