...
for (const fixture of fixtures) {
    test(`Phase B: ${fixture.name}`, () => {
        ...
        strictEqual(diff.newly_destructive, fixture.expectedNewlyDestructive, `newly_destructive should be ${fixture.expectedNewlyDestructive}`);
        // Check risk level
        strictEqual(diff.riskLevel, fixture.expectedRiskLevel, `riskLevel should be ${fixture.expectedRiskLevel}`);
        // Render the diff
        const rendered = renderDiff(diff);
        ok(rendered, 'renderDiff should produce output');
        // Extract summary for assertion
        const summary = extractSummary(diff);
        ok(summary.toLowerCase().includes(fixture.expectedSummary.toLowerCase()), `Summary should include "${fixture.expectedSummary}". Got: "${summary}"`);
        ...
        ok(renderedLower.includes(summaryLower), `Rendered output should contain summary phrase`);
    });
}
