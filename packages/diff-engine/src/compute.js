...
export function computeDiff(from, to) {
    ...
    const newly_destructive = capabilityChanges.added.some(c => c.severity === 'categorical_acquisition') ||
    ...
    const hasChanges = capabilityChanges.added.length > 0 ||
 ...
 */
function indexCapabilitiesByTool(capabilities) {
    const index = new Map();
 ...
 */
function computeCapabilityChanges(fromByTool, toByTool) {
    const added = [];
    const removed = [];
    const modified = [];
 ...
 */
function classifyAddedCapability(toolId, toCap, fromCaps) {
 ...
 */
function classifyScopeChange(fromResource, toResource, verb) {
    if (fromResource === toResource) {
 ...
 */
function isScopeWider(fromScope, toScope) {
    ...
    if (fromParts[0] !== toParts[0])
        return false; // Different resource domains
 ...
 */
function determineRiskLevel(capabilityChanges, secretChanges, newly_destructive) {
    // No changes at all
    if (capabilityChanges.added.length === 0 &&
        capabilityChanges.removed.length === 0 &&
        capabilityChanges.modified.length === 0 &&
        secretChanges.new.length === 0 &&
        secretChanges.reused.length === 0 &&
