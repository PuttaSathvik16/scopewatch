...
export class LifecycleEngine {
            ...
            if (unresolved.checkpoint_id) {
                const restore = this.db.transaction(() => {
                    ...
                    const checkpoint = this.db
                        ...
                        .get(unresolved.checkpoint_id);
                    this.db
                        .prepare(`INSERT INTO lifecycle_events
                (server_id, client_id, from_state, to_state, event_type, resolves_intent_id, checkpoint_id, created_at)
               VALUES (?, ?, ?, ?, 'recovery', ?, ?, ?)`)
                        .run(server_id, client_id, unresolved.to_state, checkpoint.state_at_checkpoint, unresolved.id, unresolved.checkpoint_id, now());
                });
                restore();
            }
            else {
    ...
    getUnresolvedIntentInternal(server_id, client_id) {
        const intent = this.db
            .prepare(`SELECT * FROM lifecycle_events
         WHERE server_id = ? AND client_id = ? AND event_type = 'transition_intent'
         ORDER BY id DESC LIMIT 1`)
            .get(server_id, client_id);
        if (!intent)
            return null;
    ...
    getUnresolvedIntent(server_id, client_id) {
     ...
     */
    startTransition(server_id, client_id, from_state, to_state, diff_id, pending_side_effects = []) {
        this.ensureRecovered();
        const existing = this.getUnresolvedIntent(server_id, client_id);
        if (existing) {
            throw new Error(`Transition already in progress for ${server_id} on ${client_id} ` +
        ...
        const run = this.db.transaction(() => {
            ...
            if (from_state === 'active') {
                ...
                if (current) {
                    const result = this.db
                        .prepare(`INSERT INTO rollback_checkpoints
           ...
           VALUES (?, ?, ?, ?, 'transition_intent', ?, ?, ?, ?)`)
                .run(server_id, client_id, from_state, to_state, diff_id, pending_side_effects.length > 0 ? JSON.stringify(pending_side_effects) : null, checkpoint_id, now());
     ...
     */
    confirmTransition(intent_id, manifest_id) {
        this.ensureRecovered();
        const intent = this.db
            .prepare('SELECT * FROM lifecycle_events WHERE id = ?')
            .get(intent_id);
        if (!intent)
            throw new Error(`No such intent: ${intent_id}`);
        const run = this.db.transaction(() => {
            this.db
                .prepare(`INSERT INTO lifecycle_events
           ...
           VALUES (?, ?, ?, ?, 'transition_confirmed', ?, ?, ?)`)
            ...
            this.db
                .prepare(`INSERT INTO lockfile_entries (server_id, client_id, manifest_id, state, last_diff_id, activated_at, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, ?)
           ON CONFLICT(server_id, client_id) DO UPDATE SET
             ...
             updated_at = excluded.updated_at`)
                .run(intent.server_id, intent.client_id, effectiveManifestId, intent.to_state, intent.diff_id, intent.to_state === 'active' ? now() : null, now());
        });
        run();
    }
    /** Resolve an intent as failed. Does NOT change lockfile_entries; from_state stands. */
    failTransition(intent_id, error) {
        this.ensureRecovered();
        const intent = this.db
            .prepare('SELECT * FROM lifecycle_events WHERE id = ?')
            .get(intent_id);
    ...
    currentManifestId(server_id, client_id) {
        const row = this.db
            .prepare('SELECT manifest_id FROM lockfile_entries WHERE server_id = ? AND client_id = ?')
            .get(server_id, client_id);
        if (!row)
            throw new Error(`No lockfile entry for ${server_id}/${client_id}; manifest_id must be supplied`);
        return row.manifest_id;
    }
    /** Shared restore logic. Internal, no guard - callers must already hold ensureRecovered(). */
    recoveryRestoreInternal(server_id, client_id, checkpoint_id) {
        const checkpoint = this.db
            .prepare('SELECT * FROM rollback_checkpoints WHERE id = ?')
            .get(checkpoint_id);
        if (!checkpoint)
            throw new Error(`No such checkpoint: ${checkpoint_id}`);
        this.db
            .prepare(`UPDATE lockfile_entries SET state = ?, manifest_id = ?, updated_at = ?
     ...
     */
    rollback(server_id, client_id) {
        this.ensureRecovered();
        const unresolved = this.getUnresolvedIntent(server_id, client_id);
        if (unresolved) {
            throw new Error(`Cannot rollback ${server_id} on ${client_id}: a transition is still unresolved ` +
                `(intent id ${unresolved.id}). Recover it first.`);
        }
        const checkpoint = this.db
            .prepare(`SELECT * FROM rollback_checkpoints WHERE server_id = ? AND client_id = ?
